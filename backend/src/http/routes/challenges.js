import { json } from "../respond.js";
import { readBody } from "../body.js";
import { getUser, requireUser } from "../session.js";
import * as challenges from "../../services/challenges.js";

// See routes/flashcards.js listDecks — the viewer is what reveals own drafts.
export async function listDecks(request, env) {
  const user = await getUser(request, env);
  return json({ decks: await challenges.listDecks(env, { user }) }, 200, request);
}

export async function createDeck(request, env) {
  const user = await requireUser(request, env);
  const { id } = await challenges.createDeck(
    env,
    await readBody(request, { user }),
  );
  return json({ ok: true, id }, 201, request);
}

export async function getDeck(request, env, { deckId }) {
  const user = await getUser(request, env);
  return json(await challenges.getDeck(env, { deckId, user }), 200, request);
}

export async function updateDeck(request, env, { deckId }) {
  const user = await requireUser(request, env);
  const result = await challenges.updateDeck(
    env,
    await readBody(request, { deckId, user }),
  );
  return json(result, 200, request);
}

export async function deleteDeck(request, env, { deckId }) {
  const user = await requireUser(request, env);
  return json(await challenges.deleteDeck(env, { deckId, user }), 200, request);
}

export async function addCard(request, env, { deckId }) {
  const user = await requireUser(request, env);
  const { id } = await challenges.addCard(
    env,
    await readBody(request, { deckId, user }),
  );
  return json({ ok: true, id }, 201, request);
}

export async function updateCard(request, env, { cardId }) {
  const user = await requireUser(request, env);
  const result = await challenges.updateCard(
    env,
    await readBody(request, { cardId, user }),
  );
  return json(result, 200, request);
}

export async function deleteCard(request, env, { cardId }) {
  const user = await requireUser(request, env);
  return json(await challenges.deleteCard(env, { cardId, user }), 200, request);
}

export async function publish(request, env, { deckId }) {
  const user = await requireUser(request, env);
  const result = await challenges.publish(env, { deckId, user });
  return json({ ok: true, ...result }, 201, request);
}
